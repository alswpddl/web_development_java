...
/*
    ...
    2. eqauls() : 객체의 동등성 표현
        ...
        형식 :
            객체명1.eqauls(객체명2)   -> 객체명1과 객체명2의 참조 주소가 동일한지 확인 : 3형식 문장

    3. hashCode() : 객체의 고유 값
        객체를 정수 값(해시값)으로 반환
        해시값은 주로 해시 기반 컬렉션에서 사용됩니다
    (Collections 파트에서 hashMap, hashSet 개념을 배울 때 언급 예정)

        eqauls()를 override하면 hashCode()도 '반드시 override'해야 합니다.
            - 동일한 객체(equals() == true)는 동일한 해시코드를 가져야만 하기 때문인데, 문제는
            - 해시 코드가 같다고 해서 항상 eqauls()가 true인 것은 아니라는 점입니다.

 */
public class Student {
    /*
        1. Student 클래스에 학생의 이름(name)과, 학번(studentId)를 저장하는 필드를 작성하세요.
        2. equals(), hashCode() 메서드를 재정의하여 학생 객체들이 같은 여부를 이름과 학번으로 비교하도록 작성하세요.

        지시 사항

            1. Studnet 클래스는 name과 studentId를 private 접근 지정자로 가지고 있어야 함.
            2. AllArgsConstrucotr로 생성자를 만들어야 함.
                - student1 - name1 = "안근수" / studentId = 20241213
