...
public class Samsung {
    ...
    // 여기서부터 싱글톤 패턴을 작성합니다.
    // 1. 스태틱 변수 정의     -> 보통 싱글톤 만들 때 객체명은 instance로 쓰는 편
    private static Samsung instance = null;

    // 2. 기본 생성자 정의 -> 접근지정자 rpivate도 주목하세요
    private Samsung() {
        company = getClass().getSimpleName();       // -> 클래스명을 가지고 오는 거였죠.
        serialNumber = 20240000;
    }

    // 3. static 메서드를 정의 -> 대부분의 경우 getInstace()로 정의함 //public 주목하세요
    public static Samsung getInstace() {
        if (instance == null) { // 현재 인스턴스가 없다면 이하의 코드가 실행됨
            instance = new Samsung();   // Samsung클래스의 객체를 생성해
        }                               // static 변수인 instace에 대입
        // 만약에 이상의 조건문이 실행되지 않는다면 이미 instace가 있다는 의미
        // 즉 getInstace() 메서드는 실행되기만 하면 어쩃든 Samsung 클래스의 객체가
