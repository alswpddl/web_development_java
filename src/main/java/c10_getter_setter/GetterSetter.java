...
/*
    1. Setter / Getter의 필요성
        ...
        이름(name), 나이(age) 등의 정보를 저장하기 위해필드를 선언했습니다.
        ...
        이처럼 외부에서 직접 접근이 가능했을 때 이상한 값 들어가더라도 통제할 수 있는
        ...
        2. Setter / Getter란?
            ...
            Getter
                데이터를 "조회"하는 역할을 하는 "method". 클래스 외부에서 변수를 직접 읽기
                못하도록 하고, 특정 메서드를 통해 안전하게 값을 반환합니다.

                Person.java / PersonMain.java를 확인 할 것 -> 응용 코드 작성

                수업 내용 확인 위해 파일 내부를 검색하려면 -> ctrl + f
                여태까지 수업한 전체 패키지 및 파일 확인에서 내용 확인 하려면 -> ctrl + shift + f

                해결책 : 캡슐화(encapsulation)
                클래스 내부의 데이터를 외부에서 바로 접근하지 못하게 하고,
                대신 "메서드"를 통해서 간접적으로 접근하도록 만드는 것.

                "이를 통해데이터의 유효성을 검증하거나 특정한 로직을 추가"할 수 있습니다.
