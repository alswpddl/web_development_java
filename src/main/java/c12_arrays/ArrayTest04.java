...
/*
    ...
    1. index가 10개인 배열을 만들고 Math.random()을 이용하여 값을 집어넣으시오 -> 1~ 10 까지의 int 배열을 만들 것
    2. 값이 입력된 배열을 출력할 것
    3. 해당 배열의 합을 구할 것
    4. 짝수만 추출해서 합을 구할 것
 */
...
//
//        // 이상의 메서드를 응용해서 1~ 10까지의 int가 출력되록 작성하시오.
...
//        System.out.println("배열 내 짝수 요소의 합: " + sumEven);
        ...
        // 이상에서 확인할 수 있듯 반복문의 구조가 데이터 입력 당시 / 합을 구할 때/ 짝수 합을 구할 때 동일하다는 것을 볼 수 있습니다.
        for (int i=0; i< numbers.length; i++){
            // 1. 데이터를 입력하는 부분
            numbers[i]=(int)(Math.random()*10)+1;
            // 3. 전체 합 구하는 부분
