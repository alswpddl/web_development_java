...
public class Method01 {
    /*
        ...
        함수와 메서드가 동일한 개념이라면 왜 용어를 분리하는가?
            -> 클래스 내부에 함수를 저의하게 되면 이를 메서드라고 침함.
            -> 자바의 경우는 모든 함수들이 특정 클래스에 종속돼있기 때문에
               전부 다 메서드에 해당함
       ...
       메서드 호출(call) : main method가 있는 곳에서 클래스명.ㅁ서드명() 혹은,
       ...
       입력해줘야만 합니다. 하지만, '사진을 찍는다'라는 메서드 내에 1)-5) 명렁어들을
       미리 입력(메서드 정의)해놓고 나서, '사진을 찍는다'라는 명령어를 실행(메서드 호출)시
       자동으로 1)-5)까지의 명렁을 순서대로 수행함
    ...
    // 3번 유형 -> [ x | o ] : void가 아니라 출력(reutrn)의 자료형을 명시해줌
    ...
    // 4q번 유형 -> [ o | o ] // call2()가 하나의 매개변수만 가졌던 것과 달리 call4()는 다수의 매개변수를 지니고 있음
    public static String call4(int year, int month, int date){
        /**
         *  DocString : 클래스나 메서드에 대한 설명을 하기 위한 처리 방법
...
//        System.out.println(todayDate);      // 실행 방법 2 : 별도의 변수를 선언하여 메서드의 return값을 저장 후, 변수를 통해 sout 실행
        ...
        System.out.print("당신이 태어난 날짜는 언제입니까? 숫자만 쓰세요. >>> ");
        int date = scanner.nextInt();

        String myBrithday = call4(year, month, date);
