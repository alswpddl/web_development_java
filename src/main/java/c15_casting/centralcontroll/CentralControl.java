...
public class CentralControl {
    ...
    // 향상된 for문을 이용하여 powerOff2() 메서드를 구현하시오 -> 이후 Main에서 powerOff2()메서드 실행
    ...
    public void displayInfo2() {
        ...
        for (int i=0; i< deviceArray.length; i++){
            ...
            if (deviceArray[i]==null){
                System.out.println("슬록[" + i + "]번 : 비어있음");
                continue;       // 현재 반복을 종료하고 i++시킨 다음 반복문으로 넘어가라는 의미
            }
            System.out.println("슬록[" + i + "]번 :" + deviceArray[i].getClass().getSimpleName());
        }
    }
    ...
    public void performSpecificMethod() {
        for (Power device : deviceArray) {
            if (device instanceof Computer) {
                ...
                // (그런데 인터페이스를는 객체 생성 불가능)이거나 혹은
                // Power 인터페이스를 상속 받은 클래스의 객체에 해당합니다.
                Computer computer = (Computer) device;  // 실행이 안되면 Computer 클래스로 다운캐스팅
                // 이제 객체명 computer는 Computer 클래스의 인스턴스이므로 Computer 클래스에 정의한
                // 고유 메서드를 실행 가능
                computer.compute();
